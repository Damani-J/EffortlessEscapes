...
async def fetch_weather(session, city, date):
    try:
        ...
        lat, lng = coordinates  # Unpack the coordinates tuple
...
async def fetch_location_photo(session, location_id):
    try:
        ...
        async with session.get(url, params=params) as response:
            ...
            # Check if we have valid photos data
            if data.get('data'):
                first_photo = data['data'][0]  # Get the first photo
                # Return the URL of the medium-sized photo (you can change to another size if needed)
...
def search():
    ...
    async def fetch_all_data(destination, start_location, departure_date, num_adults):
        async with aiohttp.ClientSession() as session:
            ...
            # Fetch weather and flight prices concurrently
            ...
            average_rating, review_text = await reviews_task  # Fetch both average rating and review text
            location_photo_url = await photo_task  # Fetch the photo URL
